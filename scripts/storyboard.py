...
def _prompt(script: str, beats: list[str], era_tags: list[str],
    ...
    return (
        ...
        "3-EVENT. SOMETHING IS HAPPENING IN THE FRAME, AND IT IS THE FIRST "
        "THING YOU WRITE. Not a person who feels something — an ACTION caught "
        "mid-movement, at the instant it can no longer be undone. This is the "
        "difference between the videos people watch and the ones they scroll "
        "past, and it is the rule a real gallery of sixteen shots broke "
        "thirteen times: every one of them was one to three figures standing "
        "upright, facing the viewer, doing nothing. \"Three villagers look "
        "worried\" obeys every other rule on this list and is a photograph of "
        "nobody doing anything.\n"
        "Write the verb first and let the subject follow it: the table goes "
        "over and coins scatter across the floor; the door is kicked in and "
        "the lamp swings; water pours past the mouth of the cave while two "
        "figures back away from it; a hand slams the ledger shut mid-sentence; "
        "the last sack is dragged out through the doorway. If a shot cannot be "
        ...
        "or a set of thumbnails. A live run returned two shots that were "
        "six-panel contact sheets of the same stick figure repeated, each "
        "panel captioned, because the line named several people and the shot "
        "was written as a list of them. A list of people renders as a sheet of "
        "people. Put them in one room, doing one thing, at one moment.\n"
        ...
        "face on the announcement, the grief after the loss, and the upturned "
        "one on the BEFORE — the boom, the deal signed, the year the plan was "
        "working. A fall needs a height to fall from, and a sequence that is "
        "grim in every frame has drawn the aftermath without the loss. Four of "
        "the five faces above are dark or blank and only one is not, so a "
        "sequence picked without thinking is grim by default: if nothing in "
        "this script is ever going well for anyone, say so deliberately rather "
        "than by omission. A sequence where "
        ...
        "7b. STAY IN THE PLACE, AND ADVANCE IT. Consecutive shots in one "
        "location are the same world a moment later, not a new drawing: the "
        "waterhole is still there, the same animal has moved, someone has "
        "arrived or left. Say what CHANGED — \"the zebra now bolting away from "
        "the same pool\", \"the stall now empty, the crates gone\" — rather "
        "than describing the place again from scratch. A sequence that keeps "
        "one world and moves things through it is what makes a viewer feel "
        "they are watching something rather than paging through pictures.\n"
        "7c. A BARE FRAME IS A DELIBERATE PUNCHLINE, USED ONCE OR TWICE. For a "
        "beat that is a comment rather than a moment — the shrug, the aside, "
        "the rhetorical question — you may write \"no background, plain empty "
        "space\" and let the figure carry it alone. That works BECAUSE the "
        "rest of the sequence is a full scene, so use it sparingly; a bare "
        "frame is a choice when it is rare and a mistake when it is the "
        "default.\n"
