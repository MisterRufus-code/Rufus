...
def init_db():
    with _conn() as c:
        ...
        """)
        # Migrate existing DBs that predate the script_full column
        try:
            c.execute("ALTER TABLE videos ADD COLUMN script_full TEXT")
        except Exception:
...
def save_video(niche: str, script_hook: str, scene_desc: str,
               ...
               script_full: str = None) -> int:
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO videos (niche, script_hook, script_full, scene_desc, youtube_id, video_file, score) "
            "VALUES (?,?,?,?,?,?,?)",
            (niche, script_hook, script_full, scene_desc, youtube_id, video_file, score),
        )
