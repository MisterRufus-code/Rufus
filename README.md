...
| `RUFUS_STILLS_TEMPLATE` | `0`/`1` | `1` | **Swap the image model with zero code.** Export ANY ComfyUI image workflow (Z-Image-Turbo, Qwen-Image, FLUX.2, …) with the positive prompt set to `RUFUS_PROMPT` to `config/stills_api.json`; Rufus uses it for every still, falling back to the built-in FLUX.1 graph (same seed) on any failure. Inert until the export exists. |
...
- **`comfy`** — local **ComfyUI + FLUX.1-dev**, the top-quality engine (needs ~24GB VRAM, e.g. **RTX 3090**). One photoreal image per beat at 832×1472 → Lanczos upscale → crop 1080×1920, with perceptual-hash dedup so no scene repeats. Start ComfyUI with `--listen` and drop `flux1-dev-fp8.safetensors` in `models/checkpoints/`. Tune via `COMFY_HOST`/`COMFY_MODEL`. Falls back: **comfy → sd → diffusers → pexels**.
  ...
  - **Swappable stills model (recommended upgrade path)**: the image model is swappable with zero code via a one-time ComfyUI export to `config/stills_api.json` (positive prompt = `RUFUS_PROMPT`, portrait ~832×1472); any failure falls back to the built-in FLUX.1 graph with the same seed, so an upgrade can never cost a clip. Recommended for this rig (RTX 3090, 16GB RAM), in order: **Qwen-Image-2512** (Apache 2.0, best quality + kills the "AI look", fp8 ~20GB — runs cleanly because stills are generated as a *separate phase* before any video model loads; see two-phase note below) or **Z-Image-Turbo** (Apache 2.0, 6B, fits fully in VRAM with no offload, portrait specialist, ~10× faster). ⚠ Avoid FLUX.2-dev here: its 24B text encoder wants 32-64GB system RAM (16GB is the blocker), and its weights are non-commercial. **Two-phase generation** (built in): all stills render first with the image model resident, then one ComfyUI `/free` unloads it, then the video model loads into a clean card — so a 20GB image model and a 28GB Wan never fight for 24GB, and ComfyUI swaps models once per video instead of ten times.
